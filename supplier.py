...
class supplierClass:
    def __init__(self, root):
        ...
        self.var_sup_invoice_id=StringVar()
        ...
        txt_search=Entry(self.root,textvariable=self.var_searchtxt,font=("goudy old style",15),bg="lightblue").place(x=800,y=80,width=160)
        ...
        txt_supplier_invoice=Entry(self.root,textvariable=self.var_sup_invoice_id,font=("goudy old style",15),bg="lightblue").place(x=180,y=80,width=180)
        
        #===row2===#
        lbl_name=Label(self.root,text="Name",font=("goudy old style",15),bg="white").place(x=50,y=120)
        txt_name=Entry(self.root,textvariable=self.var_sup_name,font=("goudy old style",15),bg="lightblue").place(x=180,y=120,width=180)

        #===row3===#
        lbl_contact=Label(self.root,text="Contact",font=("goudy old style",15),bg="white").place(x=50,y=160)
        txt_contact=Entry(self.root,textvariable=self.var_sup_contact,font=("goudy old style",15),bg="lightblue").place(x=180,y=160,width=180)
...
#==============================================================================================================
    ...
    def add(self):
        ...
        cur=con.cursor()
        try:
            if self.var_sup_invoice_id.get()=="":
                messagebox.showerror("Error","Invoice must be required",parent=self.root)
            else:
                cur.execute("Select * from supplier where invoice=?",(self.var_sup_invoice_id.get(),))
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Invoice no. already assigned, try different",parent=self.root)
                else:
                    cur.execute("Insert into supplier (invoice,name,contact,desc)values(?,?,?,?)",(
                                                self.var_sup_invoice_id.get(),
...
#==============================================================================================================
    ...
    def get_data(self,ev):
        ...
        row=content['values']
        # print(row)
        self.var_sup_invoice_id.set(row[0])
        ...
        try:
            if self.var_sup_invoice_id.get()=="":
                ...
                if row==None:
                    ...
                    if op==True:
                        cur.execute("delete from supplier where invoice=?",(self.var_sup_invoice_id.get(),))
                        con.commit()
